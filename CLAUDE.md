...
| `configs/gamestick_rk3032_defconfig` | **GENERATED by `./build.sh configure` — ne jamais éditer directement.** Toute modification de la config Buildroot (packages, flags) doit être faite dans le heredoc `cmd_configure()` de `build.sh`. |
